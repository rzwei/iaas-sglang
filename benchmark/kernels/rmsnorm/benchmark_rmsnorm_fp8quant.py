...
from vllm._custom_ops import scaled_int8_quant, scaled_fp8_quant, rms_norm_dynamic_per_token_quant
...
def unfused_int8_impl(rms_norm_layer: RMSNorm, x: torch.Tensor,
    ...
    # Quant
    torch_out, _, _ = scaled_int8_quant(torch_out)
...
def unfused_fp8_impl(rms_norm_layer: RMSNorm, x: torch.Tensor,
    ...
    # Quant
    torch_out, _ = scaled_fp8_quant(torch_out)
...
def fused_impl(
        ...
        residual: Optional[torch.Tensor],
        quant_dtype: torch.dtype):
    out, _ = rms_norm_dynamic_per_token_quant(x,
